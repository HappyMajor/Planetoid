...
namespace Mirror
{
    ...
    public class NetworkTransformUnreliable : NetworkTransformBase
    {
        ...
        public Quaternion lastSendRotation = Quaternion.identity;
        public Vector3 lastSendPosition = Vector3.zero;
        public Vector3 lastSendScale = Vector3.zero;
        ...
        void UpdateServerBroadcast()
        {
                ...
                (syncDirection == SyncDirection.ServerToClient || IsClientWithAuthority))
            {
...
#if onlySyncOnChange_BANDWIDTH_SAVING
                cachedSnapshotComparison = CompareLastSend(snapshot);
                ...
                Quaternion rotation;
                Vector3 scale;
                if(syncPosition && positionChanged)
                {
                    position = snapshot.position;
                    lastSendPosition = position;
                } else
                {
                    position = lastSendPosition;
                }
                if(syncRotation && rotationChanged)
                {
                    rotation = snapshot.rotation;
                    lastSendRotation = rotation;
                } else
                {
                    rotation = lastSendRotation;
                }
                if(syncScale && scaleChanged)
                {
                ...
                {
                    //Even if only the position changed then it will go here and save all current transform values into last snapshot.
                    //This causes a problen when the sensitivity of rotation is higher than of the position. 
                    //The rotation will never accumulate over the sensitivity because it gets set to the current new rotation and if it had a delta < sensitivity, then
                    //it will never reach the treshhold.
                    //lastSnapshot should maybe only include transform values which actually got sent?
                    hasSentUnchangedPosition = false;
                    if(!rotationChanged)
                    {
                        snapshot.rotation = lastSnapshot.rotation;
                    }
        ...
        protected virtual bool CompareLastSend(TransformSnapshot currentSnapshot)
        {
            positionChanged = Vector3.SqrMagnitude(lastSendPosition - currentSnapshot.position) > positionSensitivity * positionSensitivity;
            rotationChanged = Quaternion.Angle(lastSendRotation, currentSnapshot.rotation) > rotationSensitivity;
            scaleChanged = Vector3.SqrMagnitude(lastSendScale - currentSnapshot.scale) > scaleSensitivity * scaleSensitivity;
